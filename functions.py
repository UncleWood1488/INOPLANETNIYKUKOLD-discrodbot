...
from blackjack import blackjack, BJView
...
async def addfish(ctx, member: discord.Member, fish_type: str, amount: int = 1):
    if not any(role.id in MOD_ROLE_IDS for role in ctx.author.roles) and not ctx.author.guild_permissions.administrator:
        return await ctx.reply("❌ У вас нет прав на использование этой команды!", ephemeral=True)

    valid_types = ['cod', 'salmon', 'tropical', 'squid']
    if fish_type not in valid_types:
        return await ctx.reply(f"❌ Неверный тип рыбы. Доступно: {', '.join(valid_types)}", ephemeral=True)

    if amount <= 0:
        return await ctx.reply("❌ Количество должно быть больше 0!", ephemeral=True)

    try:
        db.add_fish_to_user(member.id, fish_type, amount)

        fish_info = fish_data.get(fish_type, {'name': fish_type, 'emoji': ''})
        await ctx.reply(
            f"✅ Выдано {fish_info['emoji']} **{fish_info['name']}** x{amount} "
            f"пользователю {member.mention}"
        )
    except Exception as e:
        logger.error(f"[ADDFISH] Ошибка: {str(e)}")
        await ctx.reply("⚠️ Ошибка выдачи рыбы", ephemeral=True)
...
async def balance(ctx):
    try:
        ...
        emb.add_field(
            name="Баланс",
            value=f"{balance} {coin_text} {SKUFCOIN_EMOJI}",
            inline=False
        )
...
async def bj(ctx, bet: int):
    ...
    try:
        ...
        view = BJView(ctx.author.id, game)
        view.bjplayers = bjplayers

        embed = game.prepare_message()
        message = await ctx.reply(embed=embed, view=view, ephemeral=False)
        view.message = message
...
async def fishing(ctx):
    try:
        ...
        catch_result = db.fishing(user_id)
        db.set_cooldown(user_id, 'fishing', cooldown['fishing'])

        fish_type = catch_result['fish_type']
        rarity = catch_result.get('rarity', 'common')
        has_lootbox = catch_result['lootbox']

        embed = create_fish_embed(ctx.author, fish_type, rarity)

        if has_lootbox:
            if embed.description:
                embed.description += "\n\n📦 **Вам выпал лутбокс!** Откройте его в магазине `/shop`"
            else:
                embed.description = "📦 **Вам выпал лутбокс!** Откройте его в магазине `/shop`"
